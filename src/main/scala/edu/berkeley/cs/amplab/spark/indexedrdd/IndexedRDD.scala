  ...
  def multiget(ks: Array[K]): Map[K, V] = {
    ...
    val ksByPartition =
      if (partitioner.isDefined)
        ks.groupBy(k => partitioner.get.getPartition(k))
      else
        partitionsRDD.partitions.indices.map(i => (i, ks)).toMap

//    val ksByPartition = ks.groupBy(k => partitioner.get.getPartition(k))      //get each key's partition
    val partitions = ksByPartition.keys.toSeq                                 //get the seq of partitions
    ...
    val results: Array[Array[(K, V)]] = context.runJob(partitionsRDD,
      (context: TaskContext, partIter: Iterator[IndexedRDDPartition[K, V]]) => {
        //only if key's partitions contain current partition that query will be executed
  ...
  def multifilter(v: Vector, r: Double, ks: Array[K], distance: VectorDisctance): Array[K] = {
    val ksByPartition =
      if (partitioner.isDefined)
        ks.groupBy(k => partitioner.get.getPartition(k))
      else
    ...
    val partitions = ksByPartition.keys.toSeq                                 //get the seq of partitions
    // TODO: avoid sending all keys to all partitions by creating and zipping an RDD of keys
    val results: Array[Array[K]] = context.runJob(partitionsRDD,
        (context: TaskContext, partIter: Iterator[IndexedRDDPartition[K, V]]) => {
          //only if key's partitions contain current partition that query will be executed
          if (partIter.hasNext && ksByPartition.contains(context.partitionId())) {
            val part = partIter.next()
            val ksForPartition = ksByPartition.get(context.partitionId()).get
            part.multiget(ksForPartition.iterator).filter(x => distance(v, x._2.asInstanceOf[Vector]) < r).map(_._1).toArray
          } else {
            Array.empty
          }
        }, partitions, allowLocal = true)
  ...
  def multiget_bak(ks: Array[K]): Map[K, V] = {
    val ksByPartition = ks.groupBy(k => partitioner.get.getPartition(k))      //get each key's partition
    val partitions = ksByPartition.keys.toSeq                                 //get the seq of partitions
    // TODO: avoid sending all keys to all partitions by creating and zipping an RDD of keys
    val results: Array[Array[(K, V)]] = context.runJob(partitionsRDD,
...
object IndexedRDD {
  ...
  (elems: RDD[(K, V)]): IndexedRDD[K, V] = updatable0(elems)

//  def apply[K: ClassTag : KeySerializer, V: ClassTag]
//      (elems: RDD[(K, V)]): IndexedRDD[K, V] = updatable(elems)

  def apply[K: ClassTag : KeySerializer, V: ClassTag]
  (elems: RDD[(K, V)], isCombine: Boolean): IndexedRDD[K, List[V]] = updatable1(elems)

  /**
   * Constructs an updatable IndexedRDD from an RDD of pairs, merging duplicate keys arbitrarily.
   */
  def updatable0[K: ClassTag : KeySerializer, V: ClassTag]
      (elems: RDD[(K, V)])
    : IndexedRDD[K, V] = updatable0[K, V, V](elems, (id, a) => a, (id, a, b) => b)

  def updatable1[K: ClassTag : KeySerializer, V: ClassTag]
  (elems: RDD[(K, V)])
  : IndexedRDD[K, List[V]] = updatable1[K, V, List[V]](elems, (id, a) => List(a), (id, a, b) => b :: a)

  /** Constructs an IndexedRDD from an RDD of pairs. */
  def updatable0[K: ClassTag : KeySerializer, U: ClassTag, V: ClassTag]
      (elems: RDD[(K, U)], z: (K, U) => V, f: (K, V, U) => V)
    : IndexedRDD[K, V] = {
    val partitions = elems.mapPartitions[IndexedRDDPartition[K, V]](
      iter => Iterator(PARTPartition(iter, z, f)),
      preservesPartitioning = true)
    new IndexedRDD(partitions)
  }

  def updatable1[K: ClassTag : KeySerializer, U: ClassTag, V: ClassTag]
  (elems: RDD[(K, U)], z: (K, U) => V, f: (K, V, U) => V)
  : IndexedRDD[K, V] = {
    val elemsPartitioned =
      if (elems.partitioner.isDefined) elems
      else elems.partitionBy(new HashPartitioner(elems.partitions.length))
